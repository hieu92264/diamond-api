...
class RentalIncidentController extends WorkflowController
{
    ...
    {
        $data = $request->validate([
            'rental_detail_id' => ['required', 'integer', Rule::exists('rental_details', 'id')],
        ...
        $detail = RentalDetail::query()
            ->with(['rentalSlip', 'detailItems'])
            ->findOrFail($data['rental_detail_id']);

        $belongsToDetail = $detail->detailItems->contains('inventory_item_id', (int) $data['inventory_item_id']);

        if (! $belongsToDetail) {
            throw ValidationException::withMessages([
                'inventory_item_id' => ['Item khong thuoc dong hang thue nay.'],
            ]);
        }

        $incident = DB::transaction(function () use ($detail, $data): RentalIncident {
        ...
        $this->syncRentalSlipFinancials($detail->rentalSlip->fresh());

        return $this->success(
            ['incident' => $this->transformIncident($this->baseQuery()->findOrFail($incident->id))],
            'Tao su co thue thanh cong.',
    ...
    public function resolve(Request $request, int $id): JsonResponse
    {
        ...
        $this->syncRentalSlipFinancials($incident->rentalDetail->rentalSlip->fresh());

        return $this->success(
            ['incident' => $this->transformIncident($this->baseQuery()->findOrFail($incident->id))],
            'Xu ly su co thue thanh cong.'
        );
    }
    ...
    public function createMaintenanceTicket(Request $request, int $id): JsonResponse
    {
        ...
        $ticket = DB::transaction(function () use ($incident, $data): MaintenanceTicket {
            ...
            $ticket = MaintenanceTicket::query()->create([
                ...
                'item_id' => $inventoryItem?->item_id ?? $incident->rentalDetail?->equipment_prop_id,
    ...
    {
        return RentalIncident::query()->with([
            'inventoryItem',
            'rentalDetail.rentalSlip',
            'rentalDetail.equipmentProp',
        ]);
    }

    private function transformIncident(RentalIncident $incident): array
    {
        ...
        return [
            ...$this->transformRentalIncidentSummary($incident),
            'detail' => [
                'slip_id' => $incident->rentalDetail?->rental_slip_id,
                'slip_code' => $incident->rentalDetail?->rentalSlip?->code,
                'detail_id' => $incident->rental_detail_id,
                'item_name' => $incident->rentalDetail?->equipmentProp?->name,
